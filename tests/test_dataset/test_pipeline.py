...
class TestPipeline(object):
    ...
    def test_load_img(self):
        results = dict(
            ...
            img_info=dict(filename='test.png'))
        transform = LoadImageFromFile()
        results = transform(copy.deepcopy(results))
        assert results['filename'] == osp.join(self.data_prefix, 'images',
                                               'test.png')
        assert results['ori_filename'] == 'test.png'
        assert results['img'].shape == (5736, 3538, 3)
        assert results['img'].dtype == np.uint8
        assert results['img_shape'] == (5736, 3538, 3)
        assert results['ori_shape'] == (5736, 3538, 3)
        assert results['pad_shape'] == (5736, 3538, 3)
        ...
        results = dict(
            img_prefix=None,
            img_info=dict(filename='tests/data/images/test.png'))
        transform = LoadImageFromFile()
        results = transform(copy.deepcopy(results))
        assert results['filename'] == 'tests/data/images/test.png'
        assert results['ori_filename'] == 'tests/data/images/test.png'
        assert results['img'].shape == (5736, 3538, 3)
    ...
    def test_load_seg(self):
        results = dict(
            seg_prefix=osp.join(self.data_prefix, 'annotations'),
            ann_info=dict(seg_map='test.png'),
            seg_fields=[])
